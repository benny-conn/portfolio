...
export async function fetchSoloTraceData() {
  ...
  try {
    ...
    const [perfsRes, jobsRes, analyticsRes] = await Promise.all([
      ...
      fetch(`${baseUrl}/api/persons/${personId}/analytics`, {
        headers: h,
        next: { revalidate: 1800 },
      }),
    ])
    ...
    const [performances, jobs, analytics] = await Promise.all([
      perfsRes.json(),
      jobsRes.json(),
    ...
    const sessions = doneJobs.map(job => {
      ...
      const longestClip = clips.reduce(
        (best, p) =>
          (p.AudioHitRatio ?? 0) > (best?.AudioHitRatio ?? 0) ? p : best,
        null,
      )
    ...
    return { latest: sessions[0], sessions, analytics }
  } catch {
    return null
  }
}
